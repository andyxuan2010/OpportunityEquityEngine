---
layout: default
title: 文档索引
locale: zh
translation_en: /docs/INDEX.md
translation_zh: /docs/INDEX.CN.md
translation_fr: /docs/INDEX.FR.md
---

# 机会公平引擎——文档索引

这是项目愿景、学生学习、研究、协作、实现和展示材料的入口。

## 推荐阅读路径

### 技术新手

1. [从这里开始](00-start-here/START_HERE.html)
2. [项目愿景](01-initiative/initiative.html)
3. [入门项目指南](03-student-guides/BEGINNER_PROJECT_GUIDE.html)
4. [快速构建应用和网站指南](03-student-guides/QUICK_APP_AND_WEBSITE_BUILD_GUIDE.html)
5. [研究与验证](07-research-and-validation/RESEARCH_AND_VALIDATION.html)
6. [机会来源目录](10-opportunity-sources/Opportunity_Equity_Engine_600_Sources.html)
7. [学生工具箱](08-student-toolkit/STUDENT_TOOLKIT.html)
8. [演示实现指南](05-build/DEMO_IMPLEMENTATION_GUIDE.html)
9. [展示与反思](09-showcase-and-reflection/SHOWCASE_AND_REFLECTION.html)

### 规划学生团队

1. [从这里开始](00-start-here/START_HERE.html)
2. [研究与验证](07-research-and-validation/RESEARCH_AND_VALIDATION.html)
3. [项目管理计划](04-delivery/PROJECT_MANAGEMENT_PLAN.html)
4. [学生工具箱](08-student-toolkit/STUDENT_TOOLKIT.html)
5. [项目时间线](04-delivery/PROJECT_TIMELINE.html)
6. [展示与反思](09-showcase-and-reflection/SHOWCASE_AND_REFLECTION.html)

### 理解技术系统

1. [系统架构](02-architecture/architecture.html)
2. [演示实现指南](05-build/DEMO_IMPLEMENTATION_GUIDE.html)
3. [学习先修知识](03-student-guides/LEARNING_PREREQUISITES.html)
4. [演示交付物](06-demo/DEMO_DELIVERABLES.html)

## 主题索引

| 主题 | 主要文档 | 内容 |
|---|---|---|
| 项目愿景 | [项目愿景](01-initiative/initiative.html)、[项目方案](01-initiative/Opportunity_Equity_Engine.html) | 目的、用户、问题、范围和产品定位 |
| 项目入门 | [从这里开始](00-start-here/START_HERE.html) | 第一个小时、角色、团队约定和完成标准 |
| 架构 | [系统架构](02-architecture/architecture.html) | 组件、数据、匹配、搜索、安全和运行 |
| 学生学习 | [入门指南](03-student-guides/BEGINNER_PROJECT_GUIDE.html)、[快速构建应用和网站指南](03-student-guides/QUICK_APP_AND_WEBSITE_BUILD_GUIDE.html)、[Vibe Coding](03-student-guides/VIBE_CODING_FOR_STUDENTS.html) | 从零开始、快速原型、AI 辅助开发和学习路径 |
| 研究与验证 | [研究与验证](07-research-and-validation/RESEARCH_AND_VALIDATION.html) | 访谈、同意、来源核验、可用性测试和证据决策 |
| 机会来源 | [600 条机会来源目录](10-opportunity-sources/Opportunity_Equity_Engine_600_Sources.html) | 600 条研究来源、核验级别、数据字段和质量规则 |
| 项目交付 | [管理计划](04-delivery/PROJECT_MANAGEMENT_PLAN.html)、[时间线](04-delivery/PROJECT_TIMELINE.html) | 角色、工具、成本、风险和里程碑 |
| 实现 | [演示实现指南](05-build/DEMO_IMPLEMENTATION_GUIDE.html) | 设置、数据模型、代码、测试和部署 |
| 协作与安全 | [学生工具箱](08-student-toolkit/STUDENT_TOOLKIT.html) | GitHub、可访问性、隐私、负责任的 AI、排障和词典 |
| 演示 | [演示交付物](06-demo/DEMO_DELIVERABLES.html) | 用户流程、产品行为和展示素材 |
| 展示与反思 | [展示与反思](09-showcase-and-reflection/SHOWCASE_AND_REFLECTION.html) | 演示故事、作品集证据、诚实表达和学习反思 |

## 文档标准

每份文档应有清晰的单一目的；新增内容应放入对应主题目录并从本索引链接。机会信息应保留来源 URL 和核验日期，同时明确区分样例数据、视觉稿和未来构想。

## 当前状态

仓库已经包含项目章程、双语项目方案、架构、学生指南、交付计划、研究材料、600 条机会来源目录、实现指南、学生工具箱和演示素材。下一步是按照 [演示实现指南](05-build/DEMO_IMPLEMENTATION_GUIDE.html) 完成垂直切片。
## 完整同步参考

以下内容会直接同步英文原文，包含完整章节、示例、链接、表格和媒体资源。

<details markdown="1">
<summary>打开完整英文参考</summary>

{% capture canonical_source %}{% include_relative INDEX.md %}{% endcapture %}
{{ canonical_source | split: 'translation_fr: /docs/INDEX.FR.md' | last }}

</details>
